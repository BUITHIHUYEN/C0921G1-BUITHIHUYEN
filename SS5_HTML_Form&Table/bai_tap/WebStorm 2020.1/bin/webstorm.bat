...
SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\JetBrains\WebStorm2020.1\webstorm%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\JetBrains\WebStorm2020.1\webstorm%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%APPDATA%\JetBrains\WebStorm2020.1\webstorm%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_WEBIDE_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_WEBIDE.hprof" -Didea.paths.selector=WebStorm2020.1 %IDE_PROPERTIES_PROPERTY%
